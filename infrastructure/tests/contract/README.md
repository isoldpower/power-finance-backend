...
The response conventions are statements about **every** endpoint, and the
services that implement them are four codebases in two languages. These tests
hold the whole surface to them at once, which is what per-service suites
structurally cannot do.

    make test-contract        # or: uv run pytest infrastructure/tests/contract -q
...
answer 507 with no write-side counterpart to be rerouted to. The reroute lands
on an unrouted path, and the gateway answers Read Service's 507 rather than a
bare 404, so the read is refused rather than reported missing. Three are left:
`GET /accounts`, `GET /accounts/{id}` and `GET /metrics`. Each is listed with
its reason.
...
prompted it. What the suite no longer does is hold the surface against a written
spec: it checks the services against each other and against the gateway, not
against a document.
