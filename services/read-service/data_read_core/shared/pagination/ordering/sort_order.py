...
class SortOrder:
    ...
    @property
    def elasticsearch_sort(self) -> list[dict[str, Any]]:
        return [key.elasticsearch_sort for key in self.keys]
    ...
    def to_elasticsearch_anchor(self, cursor_values: Iterable[Any]) -> list[Any]:
        return [
            key.codec.to_elasticsearch_value(value)
            for key, value in zip(self.keys, cursor_values, strict=True)
        ]
