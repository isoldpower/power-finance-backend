...
from .dtos import CacheOperationData, GoalDTO
from .infra import (
    CACHE_TTL_SECONDS,
    get_filter_hash,
    get_list_cache_key,
...
class CacheWorker:
    ...
    async def save_to_cache(
        ...
        cache_key = await self._build_cache_key(context)
        payload = {
            "goals": [goal.to_cache() for goal in goals],
            "total": total,
        }
    ...
    async def _try_retrieve_from_cache(
        ...
        cached_goals = [GoalDTO.from_cache(item) for item in payload["goals"]]
        return cached_goals, payload["total"]
