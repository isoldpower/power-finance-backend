...
class CacheWorker:
    ...
    async def save_to_cache(
        ...
        cache_key = await self._build_cache_key(context)
        payload = {
            "wallets": [wallet.to_cache() for wallet in wallets],
            "total": total,
        }
    ...
    async def _try_retrieve_from_cache(self, cache_key: str) -> tuple[list[WalletDTO], int] | None:
        cached_value = await self._redis_client.get(cache_key)
        if cached_value is None:
            return None

        payload = json.loads(cached_value)
        wallets = [WalletDTO.from_cache(item) for item in payload["wallets"]]
        return wallets, payload["total"]
