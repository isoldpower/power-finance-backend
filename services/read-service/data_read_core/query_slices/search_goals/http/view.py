...
from ..dtos import SearchGoalsQuery
from ..query_handler import SearchGoalsQueryHandler
from ._presenters import present_many
from ._serializers import (
    FilterGoalsRequestSerializer,
    PaginatedGoalSearchResultSerializer,
)


@extend_schema(
    operation_id="goals_search",
    summary="Search goals with filters",
    description=(
        "Retrieve goals matching a filter tree passed in the request body. "
    ...
    request=FilterGoalsRequestSerializer,
    responses={
        200: PaginatedGoalSearchResultSerializer,
        422: ErrorResponseSerializer,
    },
)
@async_api_view(["POST"])
@es_read_at_least_gate
async def search_goals(request):
    logger = get_query_logger("search_goals")
    log_request_received(
        logger,
        "search_goals",
    ...
    serializer = FilterGoalsRequestSerializer(data=request.data)
    ...
    fetched = await SearchGoalsQueryHandler().handle(
        SearchGoalsQuery(
            user_id=request.user.id,
