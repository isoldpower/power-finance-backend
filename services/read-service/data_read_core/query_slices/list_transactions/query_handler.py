...
class ListTransactionsQueryHandler:
    ...
    async def handle(self, query: ListTransactionsQuery) -> FetchedRows:
        ...
        if cached_value is not None:
            transactions, total = cached_value
            log_served_from_cache(query.user_id)
            return FetchedRows(rows=transactions, total=total, cached=True)
    ...
    async def _make_store_request(
        ...
        database_entry = await fetch_user_transactions(query.user_id, query.page)
