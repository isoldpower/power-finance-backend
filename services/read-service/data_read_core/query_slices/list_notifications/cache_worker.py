...
class CacheWorker:
    ...
    ) -> tuple[list[NotificationDTO], int] | None:
    ...
    async def save_to_cache(
        ...
        cache_key = await self._build_cache_key(context)
        payload = {
            "notifications": [notification.to_cache() for notification in notifications],
            "total": total,
        }
    ...
    async def _try_retrieve_from_cache(
        ...
        notifications = [NotificationDTO.from_cache(item) for item in payload["notifications"]]
        return notifications, payload["total"]
