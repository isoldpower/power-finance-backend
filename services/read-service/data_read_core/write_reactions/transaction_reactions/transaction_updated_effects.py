...
async def _update_transaction(payload: TransactionUpdated) -> None:
    new_amount = Decimal(payload.new_amount)
    await _apply_update(payload, new_amount)


async def _apply_update(payload: TransactionUpdated, new_amount: Decimal) -> None:
    async with aatomic():
        row = await (
