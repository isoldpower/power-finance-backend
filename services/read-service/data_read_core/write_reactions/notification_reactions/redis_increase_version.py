...
from .._cache_keys import get_notification_list_version_key
from .._logger_shortcuts import log_notification_list_version_bumped
...
class BumpNotificationListVersion(Effect):
    ...
    async def apply(self, event: EventMessage) -> None:
        ...
        version_key = get_notification_list_version_key(event_payload.user_id)
        new_version = await get_redis().incr(version_key)

        log_notification_list_version_bumped(event_payload.user_id, new_version)
