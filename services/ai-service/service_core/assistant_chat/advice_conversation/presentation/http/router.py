...
from service_core.shared.pagination import LimitSettings
...
def build_assistant_router(messages: MessageRepository | None = None) -> APIRouter:
    ...
    async def get_messages(
        ...
        limit: str | None = Query(
            None,
            description=(
                f"Page size. Defaults to {LimitSettings.DEFAULT}, clamped to "
                f"{LimitSettings.MINIMUM}..{LimitSettings.MAXIMUM} rather than refused."
            ),
        ),
