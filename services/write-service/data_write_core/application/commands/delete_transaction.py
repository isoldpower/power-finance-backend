...
class DeleteTransactionCommandHandler(CommandHandlerBase, LoadWalletMixin, LoadTransactionMixin):
    ...
    async def handle(self, command: DeleteTransactionCommand) -> TransactionDTO:
        ...
        await self._transaction_repository.create_transaction(inverse_transaction)

        wallet_aggregate = await self.load_wallet_dto(
            wallet_id=transaction_aggregate.root.source_wallet_id,
            user_id=command.user_id,
        )
        await self._publish_events(transaction_aggregate)
        return transaction_to_dto(inverse_transaction, wallet_aggregate)
