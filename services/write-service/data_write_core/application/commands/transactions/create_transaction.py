...
from data_write_core.domain.exceptions import WalletClosedError
...
class CreateTransactionCommandHandler(CommandHandlerBase[TransactionDTO], LoadWalletMixin):
    ...
    async def handle(self, command: CreateTransactionCommand) -> tuple[TransactionDTO, int]:
        ...
        if wallet_aggregate.root.deleted_at is not None:
            raise WalletClosedError(command.wallet_id)
        ...
        wallet_aggregate.record(aggregate.origin_flow)

        latest_version = await self._persist(aggregate, command.user_external_id)
    ...
    async def _persist(self, aggregate: TransactionAggregate, partition_key: str) -> int:
        return await run_transaction_saga(
            postgres_steps=[persist_transaction_step(self._transaction_repository, aggregate)],
