...
def seed_currencies(apps, schema_editor):
    CurrencyModel = apps.get_model("data_write_core", "CurrencyModel")
    CurrencyModel.objects.bulk_create(
        [
            CurrencyModel(code=code, name=name, numeric=numeric, digits=digits)
...
def unseed_currencies(apps, schema_editor):
    CurrencyModel = apps.get_model("data_write_core", "CurrencyModel")
    CurrencyModel.objects.filter(code__in=[c[0] for c in SEED_CURRENCIES]).delete()


class Migration(migrations.Migration):
    dependencies = [
        ("data_write_core", "0001_initial"),
    ]
