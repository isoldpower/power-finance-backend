...
class GetSpendingHeatmapQueryHandler:
    ...
    async def handle(self, query: GetSpendingHeatmapQuery) -> SpendingHeatmapResultDTO:
        rows = await self.transaction_selectors.get_daily_spending(user_id=query.user_id)
        data = {
